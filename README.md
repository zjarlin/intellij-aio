# AutoDDL 插件 for IntelliJ IDEA

**AutoDDL** 插件 为IntelliJ IDEA扩展一些元数据操作 。
用户可选择调用大模型自动生成Table，或手动编写表单生成 DDL。

## 意图
```kotlin
   /**
 * 1=男
 * 2:女
 * 3-其他
 */
val gender: String?
```

1. 插件提供了一些意图,比如带有枚举注释的字段上alt+回车 选择GenEnum...即可生成Java/Kotlin枚举  枚举相关在IntelliJ IDEA 设置/其他/AutoDDL设置可以看到(枚举的生成路径默认./当前目录     可以设置包路径:com.example.xxx)
2. 现在你可以在build.gradle.kts中把现有依赖upsert到libs.version.toml ,只需alt+回车



* Generate Excel DTO        根据当前类生成生成fastexcel框架的实体
* Generate Jimmer DTO     根据当前类生成生成jimmer各种DTO样板代码
* GenJimmerController       根据当前类生成controller模板(联系作者适配)
* GenAddColumnByThis    根据当前类生成ddl语句  
* GenStructuredOutput      自然语言转为当前类的json

## 功能特性

- **自动表创建**：基于结构化表单元数据生成 `CREATE TABLE` SQL 语句。
- **大模型支持**：调用大语言模型（LLM）自动生成表单元数据。
- **Java 类型映射**：自动将 Java 类型映射为数据库字段类型。
- **手动编辑**：支持对生成的表单元数据进行二次编辑。
- **手动表单编写**：手动编写表单生成 DDL，无需调用大模型。

## 使用说明

1. **插件入口**：在 **Tools** -> **AutoDDL** 中找到插件。
2. **初始配置**：在 **IDEA 设置** 中配置大模型 API Key：
   - 目前支持 **阿里灵积**（`DASHSCOPE_API_KEY`）。
3. **调用大模型生成表单**：
   - 使用大模型生成表单元数据，支持手动编辑。
4. **手动编写表单**：
   - 手动编写表单生成 DDL 语句。