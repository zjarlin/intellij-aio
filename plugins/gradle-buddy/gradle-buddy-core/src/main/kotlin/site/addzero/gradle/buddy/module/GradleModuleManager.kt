...
object GradleModuleManager {
    ...
    fun excludeModule(project: Project, modulePath: String): Boolean {
        if (isProtectedModule(modulePath)) {
            logger.warn("无法释放受保护的模块: $modulePath")
            return false
        }
        ...
        return try {
            ...
            logger.info("已释放模块: $modulePath")
            true
        } catch (e: Exception) {
            logger.error("释放模块失败: $modulePath", e)
            e.printStackTrace()
            false
        }
    }
    ...
    fun includeModule(project: Project, modulePath: String): Boolean {
        ...
        return try {
            ...
            logger.info("已包含模块: $modulePath")
            true
        } catch (e: Exception) {
            logger.error("包含模块失败: $modulePath", e)
            e.printStackTrace()
            false
        }
    }
