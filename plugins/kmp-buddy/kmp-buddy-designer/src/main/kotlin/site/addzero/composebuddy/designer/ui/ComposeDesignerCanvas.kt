...
class ComposeDesignerCanvas(
    ...
    private var selectionStartNodeId: String? = null
    ...
    private var containerSelectionMode = false
    ...
    init {
        ...
        val mouseAdapter = object : MouseAdapter() {
            override fun mousePressed(event: MouseEvent) {
                ...
                val selectionGesture = containerSelectionMode || event.isShiftDown
                selectedNodeId = hitNode?.id ?: rootNodeId
                selectionStartNodeId = hitNode?.id ?: rootNodeId
                ...
                operation = when {
                    ...
                    selectionGesture && (hitNode == null || hitNode.id == rootNodeId) -> {
                        selectionRect = Rectangle(event.point)
                        CanvasOperation.DRAW_SELECTION
                    }

                    else -> CanvasOperation.IDLE
                }
    ...
    fun unwrapSelectedContainer() {
        val selectedId = selectedNodeId ?: return
        val selected = nodes.firstOrNull { it.id == selectedId } ?: return
        if (selectedId == rootNodeId) {
            return
        }
        if (!ComposeDesignerLayoutSupport.isContainer(selected)) {
            return
        }
        val directChildren = nodes
            .filter { it.parentId == selected.id }
    ...
    private fun createContainerFromSelection() {
        ...
        if (selectionStartNodeId != null && selectionStartNodeId != rootNodeId) return
        val before = snapshotState()
