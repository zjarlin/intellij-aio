...
object ComposeDesignerWritebackSupport {
    ...
    fun findSiblingKotlinFile(
        ...
        val siblingDir = contextDirectory(project) ?: return null
    ...
    private fun currentContextFile(project: Project): VirtualFile? {
        ...
        val preferred = selectedEditor?.let { FileDocumentManager.getInstance().getFile(it.document) }
        return buildList {
            add(preferred)
            addAll(editorManager.selectedFiles.asList())
        }
            .asSequence()
            .filterNotNull()
            .mapNotNull(::toWritableLocalFileOrNull)
            .firstOrNull()
    }

    private fun contextDirectory(project: Project): VirtualFile? {
        return currentContextFile(project)?.parent ?: project.guessProjectDir()
    }

    private fun toWritableLocalFileOrNull(file: VirtualFile): VirtualFile? {
        if (!file.isValid || file.isDirectory || !file.isWritable || !file.isInLocalFileSystem) {
            return null
        }
        return file
    }
