...
class ComposeBuddyIntentionsTest : BasePlatformTestCase() {
    fun testExpandTextWrapperSignatureAddsCommonComposeParameters() {
        ...
        val action = myFixture.findSingleIntention("(KMP Buddy) Expand Compose wrapper signature")
    ...
    fun testGenerateWrapperPropsMigratesCallSites() {
        ...
        val action = myFixture.findSingleIntention("(KMP Buddy) Generate Compose wrapper props")
        action.invoke(project, myFixture.editor, myFixture.file)
    ...
    fun testNormalizeComposeSignatureCreatesPropsEventsStateAndUpdatesCalls() {
        ...
        val action = myFixture.findSingleIntention("(KMP Buddy) Normalize Compose signature")
        action.invoke(project, myFixture.editor, myFixture.file)
    ...
    fun testNormalizeComposeSignatureCanKeepCompatibilityFunction() {
        ...
        val text = myFixture.file.text
        assertTrue(text.contains("@Deprecated(\"Use the normalized kmp-buddy signature\")"))
    ...
    fun testDoesNotOfferNormalizeForNonComposableFunction() {
        ...
        val actions = myFixture.filterAvailableIntentions("(KMP Buddy) Normalize Compose signature")
        assertEmpty(actions)
    }
}
