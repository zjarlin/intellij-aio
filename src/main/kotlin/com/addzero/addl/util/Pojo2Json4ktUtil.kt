...
//                outputMap[propertyName] = getObjectForType(propertyType, project, ktClass)
//            }
//        }
//
//        return outputMap
...
//    }
    ...
    private fun getObjectForType(typeName: String?, project: Project, containingClass: KtClass, depth: Int = 0): Any? {
        ...
        return when (typeName) {
            ...
            null -> null
                ...
                handleListType(typeName, project, containingClass, depth + 1)
            } else {
                val targetClass = findKtClassByName(typeName, project)
                targetClass?.let { generateMap(it, project, depth + 1) } ?: typeName
            }
        }
    }
    private fun handleListType(typeName: String, project: Project, containingClass: KtClass, depth: Int = 0): List<Any?> {
        ...
        if (elementClass != null) {
            list.add(generateMap(elementClass, project, depth + 1))
...
//    }
    ...
    private fun findKtClassByName(className: String, project: Project): KtClass? {
        ...
        val possibleClasses = KotlinFullClassNameIndex.Helper.get(className, project, GlobalSearchScope.projectScope(project))
